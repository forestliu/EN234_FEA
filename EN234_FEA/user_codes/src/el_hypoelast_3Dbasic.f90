...
subroutine el_hypoelast_3dbasic(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    ...
    real (prec)  ::  strain(6), dstrain(6), dev_strain(6)  ! Strain vector contains [e11, e22, e33, 2e12, 2e13, 2e23]
    real (prec)  ::  new_strain(6)
    real (prec)  ::  stress(6), vol_strain                 ! Stress vector contains [s11, s22, s33, s12, s13, s23]
    real (prec)  ::  D(6,6)                            ! stress = D*(strain+dstrain)  (NOTE FACTOR OF 2 in shear strain)
    real (prec)  ::  B(6,length_dof_array)             ! strain = B*(dof_total+dof_increment)
    real (prec)  ::  dB(length_dof_array,6)             ! strain = B*(dof_total+dof_increment)
    real (prec)  ::  dxidx(3,3), determinant           ! Jacobian inverse and determinant
    real (prec)  ::  diagonal(6,6), ublock(6,6)
    real (prec)  ::  x(3,length_coord_array/3)         ! Re-shaped coordinate array x(i,a) is ith coord of ath node
    real (prec)  :: sig0, eps0, nn, KK, Vel          ! Material properties
    real (prec)  :: sige, epse, Et, Es                     ! total equivalent stress&strain
    real (prec)  ::  e_dyadic_e(6,6),strain_mag
    !
    ...
    do kint = 1, n_points
        ...
        strain_mag = dsqrt(new_strain(1)**2+new_strain(2)**2+new_strain(3)**2+ &
                            new_strain(4)**2+new_strain(5)**2+new_strain(6)**2)
        vol_strain = (new_strain(1)+new_strain(2)+new_strain(3))/3.d0
        write(*,*) 'vol_strain = ', vol_strain, '  e11 = ',new_strain(1)
...
!        write(*,*) 'epse = ', epse
        ...
        if (epse < eps0) then
            ...
            Et = (2.d0*dsqrt(nn/(nn-1.d0)-epse/eps0)/eps0) * sig0
        else
            sige = sig0 * (epse / eps0) ** (1.d0/nn)
            Et = (1.d0/nn*((epse/eps0)**(1.d0/nn-1))/eps0) * sig0
        end if
        if (strain_mag < 1.d-012) then ! zero dev strain and zero dev stress
            Es = Et
            stress(1:6) = 0
            D = Es/3.d0*diagonal + (KK-2.d0*Es/9.d0)*ublock
        else
            Es = sige / epse
            stress(1:3) = 2.d0/3.d0 * sige / epse * dev_strain(1:3) + KK * vol_strain * 3.d0
            stress(4:6) = 2.d0/3.d0 * sige / epse * dev_strain(4:6)
            D = 4.d0/9.d0/epse**2 *(Et-Es) * e_dyadic_e + Es/3.d0*diagonal + (KK-2.d0*Es/9.d0)*ublock
        end if
...
subroutine fieldvars_hypoelast_3dbasic(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    ...
    real (prec)  ::  KK,nn,sig0,eps0,diagonal(6,6),ublock(6,6),new_strain(6),epse,sige,Et,Es,e_dyadic_e(6,6)
    real (prec)  ::  vol_strain, dev_strain(6)
    !
