...
subroutine el_CahnHilliard_2dbasic(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    ...
    real (prec)  ::  DD, kk, theta, cc, dcc, mu, dmu   ! Material properties
    real (prec)  ::  dccdx(2), ddccdx(2), dmudx(2), ddmudx(2)
...
!    if (n_nodes == 20) n_points = 27
    ...
    do kint = 1, n_points
        ...
        ddmudx = 0.d0
        do i = 1, n_nodes
            cc = cc + dof_total(2*i) * N(i)
            dcc = dcc + dof_increment(2*i) * N(i)
            mu = mu + dof_total(2*i-1) * N(i)
            dmu = dmu + dof_increment(2*i-1) * N(i)
            dccdx(1) = dccdx(1) + dof_total(2*i) * dNdx(i,1)
            ddccdx(1) = ddccdx(1) + dof_increment(2*i) * dNdx(i,1)
            dccdx(2) = dccdx(2) + dof_total(2*i) * dNdx(i,2)
            ddccdx(2) = ddccdx(2) + dof_increment(2*i) * dNdx(i,2)
            dmudx(1) = dmudx(1) + dof_total(2*i-1) * dNdx(i,1)
            ddmudx(1) = ddmudx(1) + dof_increment(2*i-1) * dNdx(i,1)
            dmudx(2) = dmudx(2) + dof_total(2*i-1) * dNdx(i,2)
            ddmudx(2) = ddmudx(2) + dof_increment(2*i-1) * dNdx(i,2)
        end do
        !write(*,*) 'mu =',mu
        ! construct D
        D = 0.d0
        D(1,1) = 1.D0
        D(1,2) = 1.D0 - 3.D0 * (cc + dcc)**2.d0
        D(2,2) = 1.D0 / DTIME
        D(3,5) = -kk
        D(4,6) = -kk
        D(5,3) = DD * theta
        ...
        q(2) = dcc / DTIME
        q(3) = -kk * (dccdx(1) + ddccdx(1))
        q(4) = -kk * (dccdx(2) + ddccdx(2))
        q(5) = DD * (dmudx(1) + theta * ddmudx(1))
        q(6) = DD * (dmudx(2) + theta * ddmudx(2))
    ...
    return
end subroutine el_CahnHilliard_2dbasic


!==========================SUBROUTINE el_linelast_3dbasic_dynamic ==============================
subroutine el_CahnHilliard_2dbasic_dynamic(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    n_properties, element_properties,element_coords, length_coord_array, &                               ! Input variables
    dof_increment, dof_total, length_dof_array,  &                                                       ! Input variables
    n_state_variables, initial_state_variables, &                                                        ! Input variables
    ...
    return
end subroutine el_CahnHilliard_2dbasic_dynamic


!!==========================SUBROUTINE fieldvars_linelast_3dbasic ==============================
!subroutine fieldvars_CahnHilliard_2dbasic(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
!    n_properties, element_properties,element_coords,length_coord_array, &                                ! Input variables
!    dof_increment, dof_total, length_dof_array,  &                                                      ! Input variables
...
!        sdev = stress
!        sdev(1:2) = sdev(1:2)-p
!        !smises = dsqrt( dot_product(sdev(1:2),sdev(1:2)) + 2.d0*dot_product(sdev(3:6),sdev(4:6)) )*dsqrt(1.5d0)
...
!
!    return
!end subroutine fieldvars_CahnHilliard_2dbasic
